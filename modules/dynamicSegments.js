...
function loadHomePage(frmMainView)
{
  ...
  frmMain = frmMainView;
  frmMain.flexScroll.segFrm.widgetDataMap = {lblSeg:"lblSeg"};
  var data  = 
      [
...
function backButtonOnPages(){
  ...
  if (headerArr[1] !== null)     
    formName = headerArr[1].headerTitleLabel.text ;
  ...
  if (formName ==  SubStateEnum.MSAUnemploymentRate){
    ...
    if (headerArr[1] !== null)     
      headerArr[1].headerTitleLabel.text = MainEnum.SUBSTATEECONOMY;
...
function loadIndustryJobs(frmIndustry)
{
  ...
  frmIndustry.flexScrollSub.segSubIndustry.widgetDataMap = {lblSeg:"lblSeg"};     
  var data  = 
      [
...
function loadLabor(laborView)
{
  ...
  laborView.flexScrollSub.segSub.widgetDataMap = {lblSeg:"lblSeg"};   
...
function loadEconomy(frmEconomy)
{
  ...
  frmEconomy.flexScrollSub.segSubEconomy.widgetDataMap = {lblSeg:"lblSeg"};   
...
function loadMSA(frmMSA)
{
  ...
  frmMSA.flexScrollSub.segSubMSA.widgetDataMap = {lblSeg:"lblSeg"};   
...
function populateSegment(frmMainView)
{
  ...
  if (mystring == MainEnum.LABORFORCE){
    ...
    urlConf =  {	URL: url, requestMethod:constants.BROWSER_REQUEST_METHOD_GET};
    //frmBrowser.brwWidget.requestURLConfig = urlConf;    
    
    var ntfBrowser = new voltmx.mvc.Navigation("frmBrowser");
	ntfBrowser.navigate({urlConf:urlConf, title:mystring});
...
function LoadURLForMSA(val)
{
  ...
  switch(mystring) {
    case MSAEnum.AkronMSA:
      ...
      urlConf = {URL: bURL , requestMethod:constants.BROWSER_REQUEST_METHOD_GET}; 		
      break;      
